...
# --- Função de Conexão (SIMPLES E DIRETA) ---
# Sem cache complexo, conecta na hora para garantir que funciona
def get_db_connection():
    try:
        # Usa a senha que você já configurou
...
with tab1:
    ...
    if st.button("Salvar Insumo"):
        if nome_insumo and custo_embalagem and qtd_embalagem:
            ...
            # Correção de tipo simples (float) para não dar erro
    ...
    if st.session_state.ingredientes_temp:
        ...
        with col_act1:
            ...
            if st.button(txt_btn, type="primary"):
                ...
                # Garantindo float para não dar erro
