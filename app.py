...
# --- Conexão Persistente (ALTA VELOCIDADE) ---
# Usamos st.cache_resource para manter a conexão aberta na memória
@st.cache_resource
def init_connection():
    try:
...
def run_query(query, params=None):
    ...
    # Se a conexão caiu por inatividade, reconecta automaticamente
    if conn.closed:
    ...
    try:
        ...
        # Se der erro de conexão durante o uso, limpa o cache e tenta de novo
        st.cache_resource.clear()
        return run_query(query, params) # Tenta recursivamente uma vez
    except Exception as e:
        conn.rollback()
...
def init_db():
    ...
    # Migração (Garante que a coluna estoque_minimo existe)
        ...
        with col_act2:
            if modo_receita == "Editar Existente" and st.session_state.editando_id:
                if st.button("❌ Excluir Receita", key="btn_del_rec"):
                    ...
                    st.session_state.ingredientes_temp = []; st.session_state.editando_id = None
                    limpar_sessao(['rec_nome_in', 'rec_venda_in'])
                    st.success("Excluída!"); time.sleep(0.3); st.rerun()
...
with tab_estoque:
    ...
    if not insumos.empty:
        ...
        if st.button("Atualizar Estoque"):
            ...
            st.success(f"Estoque de {sel_mov} atualizado!"); time.sleep(0.3); st.rerun()
        ...
        with st.expander("Clique para editar nome, preço ou mínimo"):
            ...
            with st.form("form_edit_insumo"):
                ...
                if st.form_submit_button("Salvar Alterações"):
                    ...
                    st.success("Dados atualizados!"); time.sleep(0.3); st.rerun()
            ...
            if st.session_state.carrinho:
                ...
                if st.button("✅ Confirmar Pedido", key="conf_venda"):
                    ...
                    if vid:
                        ...
                        st.session_state.carrinho = []; limpar_sessao(['v_cli', 'v_end']); st.success("Pedido Feito e Estoque Atualizado!"); time.sleep(0.3); st.rerun()
    ...
    with sub_tab_vendedoras:
        ...
        with col_vend1:
            ...
            if st.button("Cadastrar"):
                if novo_nome_vend: run_query("INSERT INTO vendedoras (nome) VALUES (%s)", (novo_nome_vend,)); st.success("Cadastrada!"); time.sleep(0.3); st.rerun()
            st.divider()
                ...
                with st.expander("👜 Entregar Produtos (Sacola)"):
                    if not receitas.empty:
                        ...
                        if st.button("Entregar para Vendedora"):
                            ...
                            st.success(f"Entregue {qtd_entregar}x {prod_entregar}"); time.sleep(0.3); st.rerun()
                ...
                if not sacola.empty:
                    ...
                    with col_baixa2:
                        ...
                        if st.button("✅ Registrar Venda da Vendedora"):
                            ...
                            st.success("Venda Registrada!"); time.sleep(0.3); st.rerun()
            ...
            with st.container(border=True):
                ...
                if r['status'] == "Em Produção":
                    if c3.button("Finalizar Produção", key=f"f_{r['id']}"):
                        run_query("UPDATE vendas SET status='Concluído' WHERE id=%s", (r['id'],)); time.sleep(0.3); st.rerun()
                with c3.expander("Opções"):
                     if st.button("🗑️ Excluir Pedido", key=f"del_v_{r['id']}"):
                         run_query("DELETE FROM vendas WHERE id=%s", (r['id'],)); run_query("DELETE FROM venda_itens WHERE venda_id=%s", (r['id'],)); st.warning("Excluído"); time.sleep(0.3); st.rerun()
    ...
    if data_cx_all:
        ...
        with col_g2:
            ...
            if not df_dash.empty:
                ...
                st.line_chart(df_dash.set_index('data_movimento')['valor'])
    ...
    with st.expander("💰 Lançamento Manual (Despesas/Entradas)", expanded=True):
        with st.form("form_cx_manual"):
            ...
            if st.form_submit_button("Lançar"): 
                run_query("INSERT INTO caixa (descricao, valor, data_movimento, tipo, categoria) VALUES (%s, %s, NOW(), %s, %s)", (desc, float(val), tp, cat)); time.sleep(0.3); st.rerun()
    ...
    if not cx.empty:
        ...
        with st.expander("Gerenciar (Excluir Lançamento Errado)"):
            ...
            if st.button("Excluir Lançamento Selecionado"):
                id_to_del = int(sel_cx_id.split(" - ")[0]); run_query("DELETE FROM caixa WHERE id=%s", (id_to_del,)); st.success("Excluído!"); time.sleep(0.3); st.rerun()
