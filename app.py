...
# --- 1. CONEXÃO (SEGURA - SEM CACHE PARA EVITAR QUEDAS) ---
def get_db_connection():
    try:
        # Usa o segredo que já funciona
...
# --- 2. LEITURA DE DADOS (COM CACHE - RÁPIDO) ---
# Guarda as listas na memória por 5 min para o celular não ficar lento
...
# --- 3. GRAVAÇÃO DE DADOS (SEM CACHE - SEGURO) ---
def run_query(query, params=None):
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        try:
            ...
            # Limpa o cache para que o novo item apareça na lista
            st.cache_data.clear()
...
with tab1:
    ...
    # O st.form impede que o site recarregue enquanto você digita no celular
    with st.form("form_insumos", clear_on_submit=True):
        ...
        if submitted:
            if nome_insumo and custo_embalagem and qtd_embalagem:
                ...
                # Uso float() para garantir que não dê erro de tipo
    ...
    with st.expander("🗑️ Excluir Insumo"):
        # Usa get_data (Cache) para ser rápido
...
with tab2:
    ...
    if modo_receita in ["Clonar/Escalar", "Editar Existente"] and not receitas_existentes.empty:
        ...
        if c_sel2.button("Carregar"):
            ...
            # Leitura direta do banco para garantir dados frescos na edição
...
with tab_orc:
    ...
    with st.form("form_orc_header"):
        ...
        st.form_submit_button("Atualizar Cabeçalho")
    ...
    with sub_tab_vendedoras:
        ...
        with c_v1:
            with st.form("cad_vendedora"):
                ...
                if st.form_submit_button("Cadastrar"):
                    run_query("INSERT INTO vendedoras (nome) VALUES (%s)", (novo_nome_vend,))
        ...
        with c_v2:
            if not vendedoras_db.empty:
                ...
                # Leitura direta para garantir dados atualizados
                data_sc = run_query(f"SELECT c.id, r.nome, c.qtd_entregue, c.qtd_vendida, (c.qtd_entregue - c.qtd_vendida) as em_maos, r.preco_venda, r.id as rec_id FROM consignacoes c JOIN receitas r ON c.receita_id = r.id WHERE c.vendedora_id = {vendedora_id} AND (c.qtd_entregue - c.qtd_vendida) > 0")
                    ...
                    with st.form("baixa_form"):
                        ...
                        if st.form_submit_button("✅ Registrar Venda"):
                            ...
                            if qtd_venda_vend > dados_item['em_maos']: st.error("Qtd maior que estoque da vendedora!")
                                ...
                                if receber_agora: run_query("INSERT INTO caixa (descricao, valor, data_movimento, tipo, categoria) VALUES (%s, %s, NOW(), 'Entrada', 'Vendas')", (f"Venda #{vid} - {vendedora_sel_nome}", float(total_v)))
